...
T="heron build"
start_timer "$T"
python ${DIR}/save-logs.py "heron_build.txt" bazel --bazelrc=tools/travis-ci/bazel.rc build --config=ubuntu heron/...
end_timer "$T"

# run heron unit tests
T="heron test non-flaky"
start_timer "$T"
python ${DIR}/save-logs.py "heron_test_non_flaky.txt" bazel --bazelrc=tools/travis-ci/bazel.rc test --config=ubuntu --test_tag_filters=-flaky heron/...
end_timer "$T"

# flaky tests are often due to test port race conditions, which should be fixed. For now, run them serially
T="heron test flaky"
start_timer "$T"
python ${DIR}/save-logs.py "heron_test_flaky.txt" bazel --bazelrc=tools/travis-ci/bazel.rc test --config=ubuntu --test_tag_filters=flaky --jobs=0 heron/...
end_timer "$T"

# build packages
T="heron build tarpkgs"
start_timer "$T"
python ${DIR}/save-logs.py "heron_build_tarpkgs.txt" bazel --bazelrc=tools/travis-ci/bazel.rc build --config=ubuntu scripts/packages:tarpkgs
end_timer "$T"

T="heron build binpkgs"
start_timer "$T"
python ${DIR}/save-logs.py "heron_build_binpkgs.txt" bazel --bazelrc=tools/travis-ci/bazel.rc build --config=ubuntu scripts/packages:binpkgs
end_timer "$T"
